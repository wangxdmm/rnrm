...
test("nnrm current", async () => {
  const { stdout: preReg } = await nnrmCommand("current");
  await nnrmCommand("use", "github");
  const { stdout } = await nnrmCommand("current");
  expect(stdout).toBe("github");
  await nnrmCommand("use", preReg);
  const { stdout: resetReg } = await nnrmCommand("current");
  expect(stdout).toBe(resetReg);
});
